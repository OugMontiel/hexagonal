  ...
  validateProductData = () => {
    return [
      body('name')
        .notEmpty()
        .isString()
        .withMessage('The product name is mandatory'),
      body('price')
        .notEmpty()
        .isFloat({ gt: 0 })
      ...
      body('stock')
        .notEmpty()
        .isInt({ min: 0 })
        .withMessage('The stock must be a non-negative integer'),
      body('SKU').notEmpty().isString().withMessage('The SKU is mandatory'),
      body('brand')
        .optional()
        .isString()
        .withMessage('The brand must be a string'),
      body('available')
        .optional()
        .isBoolean()
  ...
  validateProductUpdateDataById = () => {
    return [
      body('name')
        .optional()
        .isString()
        .withMessage('The product name must be a string'),
      body('price')
        .optional()
        .isFloat({ gt: 0 })
        .withMessage('The price must be a positive number'),
