...
class UserController {
    ...
    async login(req, res) {
        try {
            ...
            const user = await this.userService.getNicknameByNickAndPassword(req.body.nick, req.body.password);
            if (!user) {
                return res.status(401).json({ error: 'Invalid credentials' });
            }
            const token = jwt.sign({ nick: req.body.nick }, 'clave', { expiresIn: '60s' });
